...
t <- nrow(prices)
n <- ncol(prices) -1#  # number of companies, minus 1 because of the date col 
R <- as.matrix(prices[2:t,2:(n+1)]-prices[1:(t-1),2:(n+1)])/(prices[1:t-1,2:(n+1)])
...
```{r}
# Annual return
r_e <- as.numeric(P_perf(R, w3, 252, rf)[2,1])
...
prices_30 <- prices %>% select(c(stocks$Symbol))

# Calculate the arithmetic daily returns
t <- nrow(prices_30)
n <- ncol(prices_30)
...
df <- data.frame(w_T, stocks$weight, stocks$Symbol) %>% filter(stocks.weight >= nth(stocks$weight, 10))

df %>% ggplot(aes(x = stocks.weight, y = w_T)) + geom_point() +
...
# Next, we compare the weights for the ten largest companies:
df <- data.frame(w_mvp, stocks$weight) %>% filter(stocks.weight >= nth(stocks$weight, 10))

df %>% ggplot(aes(x = stocks.weight, y = w_mvp)) + geom_point() +
...
```{r}
tibble(MVP, TP) %>% mutate(date = 1:length(TP),
                           MVP = MVP/MVP[1],
                           TP = TP/TP[1])  %>% 
  gather(key = "stock", value = "Price", MVP, TP) %>% 
  ggplot(aes(x = date, y = Price, color = stock)) + 
  geom_line()
```

Is the whole S&P500 the tangency portfolio? Calcu- late the weights of all 500 constitutes inside the tangency portfolio. Comment on the difference between the top10 constitutes between this tangency portfolio and 30-stock tangency portfolio from (1)
...
First note that both portfolios are efficient, this means that there is no further idiosyncratic risk that can be diversified away by combining two portfolios, and all remaining common risk is due to market risk. The remaining common risk is measured by the correlation between the two. 

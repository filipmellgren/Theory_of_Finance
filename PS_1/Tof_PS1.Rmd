...
t <- nrow(prices)
n <- ncol(prices)
R <- as.matrix(prices[2:t,2:n]-prices[1:t-1,2:n])/(prices[1:t-1,2:n]) # TODO necessary?
mu <- exp(mean(R$AAPL)*252/t)
# Define the annualised mean returns and the covariance matrix
mu <- t(exp((prices[t, 1:n]/prices[1, 1:n]-1)*252/t) -1)
mu_e <- mu - rf
Sigma <- cov(prices[2:t,1:n], prices[1:(t-1), 1:n])
```
...
sum(w_T)
#? w <- MVcalc(0.05, mu, cov)

# Finally, we compare the weights:
df <- data.frame(w_T, stocks$weight) %>% filter(stocks.weight >= nth(stocks$weight, 10))

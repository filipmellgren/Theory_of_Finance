...
# Create a new weight such that it adds up to 1
stocks <- stocks %>% mutate(weight = `Weight (%)`/sum(`Weight (%)`))

# Maintain the same companies in the other df
prices <- prices %>% select(c(stocks$Symbol))
...
# Define the mean returns and the covariance matrix
mu <- t(exp((prices[t, 1:n]/prices[1, 1:n] - 1) * 252/t) -1)
cov <- cov(prices)
```

First,

Find the tangency portfolio. I.e. a point in the mu, sigma space.


```{r}
# Define some intermediary scalars that we use for optimising the portfolio
# (These were taken from the lecture notes, lecture 3 p.9)
ones <- rep(1, n)
A <- t(mu) %*% solve(cov) %*% mu
B <- t(mu) %*% solve(cov) %*% ones
...
mu_star <- 0.05 # seq(0.01, 0.5, 0.001)
lambda <- (C * mu_star - B) /(A*C - B^2)
delta <- (A - B*mu_star) / (A*C - B^2)

w <- solve(cov) %*% (mu * lambda + ones * delta)
...
```{r}
# Finally, we compare the weights:

df <- data.frame(w, stocks$weight) %>% filter(stocks.weight >= nth(stocks$weight, 10))

df %>% ggplot(aes(x = stocks.weight, y = w)) + geom_point() +
  labs(title = "No relation between S&P weight and optimal weight", 
       x = "S&P weight", y = "Optimal weight") + 
